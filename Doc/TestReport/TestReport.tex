...
\textbf{Requirement \#23}: Being an educational tool, it was of the utmost importance that GrateBox be easy to use. To this end, a set of tasks was set before users to be completed within a set time frame, to judge the program's ease of use. While most users were able to complete the tasks in a timely manner, some were not. These users had problems understanding exactly what fitness values were in practical terms, and so could not utilize the program to the fullest. To remedy this, Grate incorporated a more detailed and nuanced explanation of the program's fitness value, and the program's ease of use increased greatly as a result.\\
...
\textbf{Initial State:} & Generation of cars created and simulation running\\
\textbf{Input:} & Invalid chromosome and invalid fitness modules commanded to send text to GUI\\
\textbf{Expected Output:} & Error message displayed\\[0.6em]
\textbf{Result of Test:} & Passed\\
\hline
...
\textbf{Initial State:} & None\\
\textbf{Input:} & Simultaneous beginning of timer and execution of car generation, mutation, and display\\
