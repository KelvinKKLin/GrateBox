...
fun and educational way. The implementtation will be completed in JavaScript.
...
requirements and for surveying representaative users for feedback and testing 
...
should be the same number of wheels as there are wheel radiuses as in 1.2.
...
How test will be performed: Car will be randomly created. All wheel radiuses 
will be found to be positive numbers within a certain range to be determined at 
a later date. Note that the output should be the same number of radiuses as 
...
Input: Incorrect car generaged from car model. (i.e. a car object where values 
...
Input: Series of car locations from simulation (x,y coords as described in PoC 
demonstration)
...
value will have been caluclated seperately and entered into the unit test prior 
...
Output: Text file that contains numerical values with lables that will be 
...
purposes (i.e. are the values we expected the values we recieved). This will 
...
users rated the visual aesthetic of the program favorably then this test would 
be a success).
...
to fill out a survey (see section 7.2) asking for their input. Test results 
shall be determined from those responses (i.e. if a majority of representative 
users rated the style of the program favorably then this test would be a 
success).
...
Output: A majority of tested users shall agree  that the program's useability is 
high.
...
a majority of representative users agree that the program's useability is high 
...
How test will be performed: Built in java script timer method will be used with 
Q-Unit (see section on unit testing) to record the time it takes for 100 
generations. If all fall below 20 seconds from beggining to end then the test 
...
will be considered a success if generated cars are a 100 precent match to the 
...
survey shall be reformated and placed on google docs at a later date.
...
functions obvious at first sight, did the program give the feedback you neeeded, 
...
throughout, did you experiance choppyness)
